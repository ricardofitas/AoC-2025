...
fn solve(input: &str) -> u64 {
    ...
    for i in 0..n {
        ...
        if a.y == b.y {
            // horizontal segment at y=a.y from xlo..xhi
            ...
            // Block crossing at this y-line for all x-intervals in [xlo, xhi)
            for cx in xs0..xs1 {
                hwall[cx + yidx * cw] = true;
            }
        } else {
            // vertical segment at x=a.x from ylo..yhi
            ...
            // Block crossing at this x-line for all y-intervals in [ylo, yhi)
            for cy in ys0..ys1 {
    ...
    // Flood fill OUTSIDE in cell space (0,0) is outside due to margin
    ...
    // Weighted prefix sum over INSIDE cells (each cell weight = #integer tiles inside it)
    let pw = cw + 1; // = xlen
    let ph = ch + 1; // = ylen
    let mut pref = vec![0u64; pw * ph];
    ...
    fn rect_sum(pref: &[u64], pw: usize, x0: usize, y0: usize, x1: usize, y1: usize) -> u64 {
        // sums cells in [x0..x1) x [y0..y1), using signed intermediates (debug-safe)
    ...
    // Check all red pairs as opposite corners; require full rectangle inside (red+green region)
    let mut best: u64 = 0;
    ...
    for i in 0..n {
        ...
        for j in (i + 1)..n {
            ...
            // half-open rectangle on coordinate lines: [xmin, xmax+1) x [ymin, ymax+1)
