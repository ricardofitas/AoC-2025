...
fn solve(input: &str) -> u64 {
    ...
    // Shapes block ends when we hit the first "WxH:" region line.
    // Input format is consistent: shape header "k:" then 3 rows, with blank lines between.
    // We'll read all lines, but stop shape parsing when we detect a region line.
    ...
    while i < buffered.len() {
        ...
        if let Some(id_str) = s.strip_suffix(':') {
            ...
            // Shouldn't happen in clean input, but don't infinite-loop.
            i += 1;
        }
    }

    // Now count how many region lines can fit by area.
    let mut ok: u64 = 0;

    // Process the region line we already buffered (if any)
    ...
    // Process remaining region lines from the iterator
    for l in lines {
        if l.is_empty() {
            continue;
        }
...
fn is_region_line(s: &str) -> bool {
    // Fast check: "<digits>x<digits>:"
    // e.g. "45x43: 21 39 38 39 38 35"
