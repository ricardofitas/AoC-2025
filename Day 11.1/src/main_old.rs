...
fn solve(input: &str) -> u128 {
    ...
    // Iterative DFS postorder from 'you' to compute DP:
    // ways[out] = 1
    // ways[v] = sum ways[child]
    // Detect reachable cycles (would imply infinite paths in general).
    let n = adj.len();
    let mut state = vec![0u8; n]; // 0=unvisited, 1=visiting, 2=done
    let mut ways = vec![0u128; n];

    let mut stack: Vec<(usize, u8)> = Vec::new(); // (node, phase) phase 0=enter, 1=exit
    stack.push((start, 0));

    while let Some((v, phase)) = stack.pop() {
        if phase == 0 {
            match state[v] {
                2 => continue, // already solved
