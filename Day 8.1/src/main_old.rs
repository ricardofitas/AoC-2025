...
// Max-heap entry. We keep K smallest edges, so the "worst of the kept" is on top.
...
fn solve(input: &str, k_req: usize) -> u128 {
    ...
    if k == 0 {
        // no connections possible => all components are size 1
        return 1;
    }

    // Keep K closest edges without storing all O(n^2).
    ...
    for a in 0..n {
        ...
        for b in (a + 1)..n {
            ...
            if heap.len() < k {
                ...
                // Current "worst among kept"
                let worst = *heap.peek().unwrap();
                // Replace if strictly better (smaller tuple)
    ...
    // Union all K selected pairs (order doesn't matter).
...
fn parse_points(input: &str) -> Vec<Pt> {
    // Fast-ish parser: each line "x,y,z"
